...
//
// This store is an Upstash "Global" (multi-region) database: writes
// land on a primary and replicate to read regions, and Upstash returns
// an Upstash-Sync-Token header so a client can ask to read from a
// replica that has caught up to a specific write instead of whichever
// one answers first. We forward that token on every request and keep
// the newest one we've seen in module scope, which survives across
// requests as long as this serverless function instance stays warm —
// it closes the gap for the common case (save, then immediately reload
// the list) but is not a substitute for a single-region database if
// replication lag turns out to be the real fix needed here.
let lastSyncToken = null;

async function kvGet(key) {
  const headers = { Authorization: `Bearer ${KV_TOKEN}` };
  if (lastSyncToken) headers['Upstash-Sync-Token'] = lastSyncToken;
  const r = await fetch(`${KV_URL}/get/${key}`, { headers });
  const token = r.headers.get('upstash-sync-token');
  if (token) lastSyncToken = token;
  let body;
