...
export default function Home({ initialSiteLocked = false }) {
  ...
  useEffect(() => {
    ...
    // Load featured text for homepage banner: falls back to most recent
    // when nothing is explicitly marked as featured, so the section is
    // never empty once at least one text exists.
    fetch('/api/texts')
      .then(r => r.json())
      .then(texts => {
        const featured = texts.find(t => t.featured)
          || texts.sort((a, b) => new Date(b.publishedAt) - new Date(a.publishedAt))[0];
        if (!featured) return;
...
<div id="page-home" class="page active">
  ...
  <section class="tom-section" id="text-of-month-section">
    <div class="tom-inner" id="text-of-month-block" style="min-height:180px"></div>
  </section>
  <!-- Newsletter -->
...
<div id="page-bundles" class="page">
  ...
  <div style="background:var(--bg-surface);padding:1.5rem 0;"><div class="wrap"><div class="flash-bar"><span class="flash-icon">⚡</span><p class="flash-text en"><strong>Flash Sale:</strong> Geopolitics Pack, 40% off for <span id="ct-en">00:00:00</span></p><p class="flash-text fr"><strong>Vente Flash :</strong> Pack Géopolitique, 40 % de réduction, <span id="ct-fr">00:00:00</span></p><p class="flash-text es"><strong>Venta Flash:</strong> Pack Geopolítico, 40% de descuento, <span id="ct-es">00:00:00</span></p><button class="btn btn-gold en" style="font-size:12px;padding:9px 18px;">Claim Offer</button><button class="btn btn-gold fr" style="font-size:12px;padding:9px 18px;">Saisir l'Offre</button><button class="btn btn-gold es" style="font-size:12px;padding:9px 18px;">Aprovechar</button></div></div></div>
